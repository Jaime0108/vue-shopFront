    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuDefaultImg="skuDefaultImg"></Zoom>
          <!-- 小图列表 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <!-- <dd changepirce="0" :class="spuSaleAttrValue.isChecked == '1' ? 'active': ''" v-for="(spuSaleAttrValue, index) in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id">{{spuSaleAttrValue.saleAttrValueName}}</dd> -->
                <!-- 绑定类名的第二种方法 -->
                <dd
                  changepirce="0"
                  ...
                  @click="
                    changSelectedSaleAttrValueName(
                      spuSaleAttrValue,
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getDetailPageInfo", this.$route.params.skuId);
    this.$bus.$on("changskuDefaultImg", (imgSrc) => {
      // 这样直接修改computed应该是不行的吧？
      // 修改state的数据应该也不行
      this.skuDefaultImg = imgSrc;
    });

    this.skuDefaultImg = this.skuInfo.skuDefaultImg || '';
  },
  data() {
    return {
      count: 1,
  ...
  watch: {
    skuInfo: {
      handler(newValue, oldValue) {
        this.skuDefaultImg = newValue.skuDefaultImg;
      },
    },
  },
  computed: {
  ...
  methods: {
    changSelectedSaleAttrValueName(spuSaleAttrValue, spuSaleAttrValueList) {
    ...
    addCart() {
      try {
        ...
        this.spuSaleAttrList.forEach((spuSaleAttr) => {
          spuSaleAttr.spuSaleAttrValueList.forEach((spuSaleAttrValue) => {
            if (spuSaleAttrValue.isChecked == 1) {
              this.selectedSaleAttrList.push(spuSaleAttrValue);
            }
          });
        });
        // 路由跳转
        ...
        // 组件通信，把选择的商品信息给加入购物车成功的页面
        window.sessionStorage.setItem(
          "addCartInfo",
          JSON.stringify({
            skuInfo: this.skuInfo,
            count: this.count,
            selectedSaleAttrList: this.selectedSaleAttrList,
          })
        );
      } catch (error) {
        console.log(error);
